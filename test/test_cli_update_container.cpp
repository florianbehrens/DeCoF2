...
using decof::update_container;

struct fixture
{
    update_container updates_;
};
...
BOOST_FIXTURE_TEST_CASE(push_single_element, fixture)
{
    ...
    update_container::time_point before = std::chrono::system_clock::now();
    updates_.push(nominal_uri, nominal_value);
    update_container::time_point after = std::chrono::system_clock::now();
...
{
    update_container::time_point time[6];
    ...
    std::string actual_uri;
    update_container::time_point actual_time;
    boost::any actual_value;
