...
BOOST_FIXTURE_TEST_CASE(empty_response_error, fixture)
{
    std::stringstream     ss;
    decof::scgi::response resp =
        decof::scgi::response::stock_response(decof::scgi::response::status_code::internal_server_error);
    ss << resp;

    std::string result = "HTTP/1.1 500 Internal Server Error\r\n"
                         "Content-Type: text/plain\r\n"
                         "Content-Length: 0\r\n\r\n";
    BOOST_REQUIRE_EQUAL(ss.str(), result);
}
...
BOOST_FIXTURE_TEST_CASE(response_with_status_no_content, fixture)
{
    ...
    std::string result = "HTTP/1.1 204 No Content\r\n"
                         "Content-Type: text/plain\r\n\r\n";
    BOOST_REQUIRE_EQUAL(ss.str(), result);
}
...
BOOST_FIXTURE_TEST_CASE(response_with_status_not_modified, fixture)
{
    std::stringstream     ss;
    decof::scgi::response resp =
        decof::scgi::response::stock_response(decof::scgi::response::status_code::not_modified);
    ss << resp;

    std::string result = "HTTP/1.1 304 Not Modified\r\n"
                         "Content-Type: text/plain\r\n\r\n";
    BOOST_REQUIRE_EQUAL(ss.str(), result);
}
